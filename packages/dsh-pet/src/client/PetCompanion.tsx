 ...
 * @module @linxin666/dsh-pet/client/PetCompanion
 */
...
/** Client-side default before the first host snapshot arrives. */
const DEFAULT_PET_ID = 'whale'

/** Browser URL of one pet asset (served by the host half's own route). */
export function petAssetUrl(petId: string, file: 'spritesheet.webp' | 'pet.json'): string {
  return `/pet/${encodeURIComponent(petId)}/${file}`
}
...
export interface PetCompanionProps {
  ...
  onPet: () => void
  /** Feed the active companion (panel button). */
  onFeed: () => void
  /** Hide the active companion (panel button). */
  onHide: () => void
 ...
 */
export function PetCompanion(props: PetCompanionProps): ReactPortal {
  const { snapshot, display, feedback } = props
  const petId = snapshot?.petId ?? DEFAULT_PET_ID
  ...
  // Load the selected atlas; then resolve per-row frame counts so tracks never
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      if (cancelled) return
      setImageReady(true)
      fetch(petAssetUrl(petId, 'pet.json'))
        ...
        .then((manifest: { frames?: unknown }) => {
          ...
          if (Array.isArray(frames)
            && frames.length === 9
            && frames.every((n) => Number.isInteger(n) && n >= 1 && n <= FRAME_COLUMNS)) {
    ...
    img.src = petAssetUrl(petId, 'spritesheet.webp')
    return () => {
      cancelled = true
      img.onload = null
      img.onerror = null
  ...
  // trailing click (fired after pointerup) does not pet the companion.
    ...
    >
      <div
        ...
        style={{
          width: spriteWidth,
          height: spriteHeight,
          backgroundImage: imageReady ? `url(${petAssetUrl(petId, 'spritesheet.webp')})` : undefined,
          // Width fixes the 8-column scale while `auto` preserves either the
          // legacy 9-row or the Codex v2 11-row atlas height.
          backgroundSize: `${FRAME_WIDTH * FRAME_COLUMNS * spriteScale}px auto`,
        ...
        role="button"
        tabIndex={0}
        aria-label={props.t('pet.spriteLabel', { name: snapshot?.name ?? props.t('pet.defaultName') })}
      />
      {feedback !== null && (
          ...
          {renaming ? (
            ...
            <>
              <div className={styles.rankRow}>
                <span className={styles.nameCell}>{snapshot?.name ?? props.t('pet.defaultName')}</span>
